...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == "update":
        df = pd.read_csv("jubeat_list.csv", index_col=None,
                         dtype={"title": str, "artist": str, "bpm": str,
                                "bsc_level": float, "adv_level": float, "ext_level": float,
                                "bsc_note": int, "adv_note": int, "ext_note": int})
        ...
        with open("jubeat_cache.pickle", "wb") as fp:
            pickle.dump(df, fp)
            print("jubeat缓存已更新！")
            exit(0)
...
@on_command('gojb', only_to_me=False)
async def go(session: CommandSession):
    ...
    with open("extensions/gojb/jubeat_cache.pickle", "rb") as fp_cache_jb:
